...
def select_free_set(inst, sa, fm, av, strategy, size):
    ...
    if strategy == "makespan_tail":
        ...
        loads = {}
        for j in range(N):
            loads[sa[j]] = loads.get(sa[j], 0.0) + max(
                (av.T_din[i][j] for i in range(M) if fm[j][i] > EPS), default=0.0)
        if not loads:
            base = list(range(N))
        else:
            s_hot = max(loads, key=lambda s: loads[s])
                ...
                if tl >= 1.0:
                    ...
                    if r is not None:
                        ...
                        if ok and incumbent_improves(av_a, av_best):
                            ...
                            accepted = 1
                            mh_consec_fail = 0
                            mh_scores[strat] = mh_scores.get(strat, 1.0) + 1.0
                            no_improve = 0
                            k = 1
        ...
        if t_deadline - time.time() > 1.0:
            ...
            def _on_sweep_improve(strat, av_new):
                ...
                submip_log.append({
                    ...
                    "free": cfg.MH_FREE_SET_SIZE, "obj_gurobi": None,
