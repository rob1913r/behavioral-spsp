...
J_LAB     = 8          # Eq. 33/35: jornada diária de trabalho (horas/dia)
H_SPAN    = 14 * J_LAB # duração da sprint em horas (14 dias × j_lab = 112h)
...
# --- Fator técnico e comportamental (Eq. 1–3 do artigo) ---
# Eq. 1: f_tech = 1 / (1 + (1/|H|)·Σ_h max(0, Hd_ih - Ht_jh))  — forma de DIVISÃO
#        (côncava, retornos decrescentes); só premia EXCEDENTE; sem coeficiente nem
#        piso (a única constante é 1/|H|). Não há F_TECH_WEIGHT/F_TECH_NORM/F_TECH_MIN.
F_BEH_WEIGHT   = 5.0     # weight do atrito comportamental na Eq. 3
PSI_WEIGHT = 0.5         # Eq. 2: psi = (1/2)·Σ_p |Pd_ip - Pt_jp|  (perfis Pd/Pt somam 1)
...
CONSTRAINT_MODE = {
    # Modelo corrigido (optimizer_corrigido): TODAS as 43 restrições do artigo
    # são ENFORCE — inclusive a Eq. 37 (cobertura por skill HdT_ijh), agora
    # presente nos 3 solvers. Ver planning/changes.md.
}
