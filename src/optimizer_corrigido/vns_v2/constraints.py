...
from optimizer_corrigido.vns.constraints import *                       # noqa: F401,F403
from optimizer_corrigido.vns.constraints import (                       # re-exporta p/ o solver
...
def build_reduced_model(inst, sa, fm, free_set, sprint_radius, dev_candidates,
    ...
    F_tot = m.addVars(triples, lb=-GRB.INFINITY, name="Ftot")   # Eq. 62: F^tot ∈ ℝ
    gamma = m.addVars([(i, j, s, f) for (i, j, s) in triples for f in _F_FRACS],
                      lb=0.0, name="gam")                        # Eq. 63: γ ≥ 0
    T_aux = m.addVars(triples, lb=0, name="Taux")   # Eq. 29/63: γ≥0 ⇒ T_aux ≥ 0
    T_din = m.addVars(triples, lb=0, name="Tdin")   # Eq. 63: T_din ≥ 0
    ...
    for (i, s) in IS:
        ...
        for a in range(len(ts)):
            for b in range(a + 1, len(ts)):
                j, k = ts[a], ts[b]
                m.addConstr(v[i, j, k, s] + v[i, k, j, s] >= w[i, j, s] + w[i, k, s] - 1)  # Eq. 42
                m.addConstr(Inic[k] >= Inic[j] + T_din[i, j, s] - _BIG_M * (1 - v[i, j, k, s]))  # Eq. 43
                m.addConstr(Inic[j] >= Inic[k] + T_din[i, k, s] - _BIG_M * (1 - v[i, k, j, s]))  # Eq. 43
                m.addConstr(v[i, j, k, s] + v[i, k, j, s] <= w[i, j, s])  # Eq. 44
                m.addConstr(v[i, j, k, s] + v[i, k, j, s] <= w[i, k, s])  # Eq. 45
    ...
    for (i, j, s) in triples:
        ...
        # Eq. 29/63: T_aux ≥ 0 (γ≥0) e T_din = T_aux + Setup
    ...
    for j in range(N):
        ...
        for s in sprs:
            devs_js = triples_by_js.get((j, s), [])
            # Eq. 31: z = Σ_i x
            m.addConstr(z[j, s] == gp.quicksum(x[i, j, s] for i in devs_js))
            # Eq. 38: máx. 4 devs por task
            m.addConstr(gp.quicksum(w[i, j, s] for i in devs_js) <= _D_MAX)
        ...
        # Eq. 36: prazo individual da task (Σ s·z <= S_max_j)
        m.addConstr(gp.quicksum(s * z[j, s] for s in sprs) <= inst["S_max_j"][j])
        # Eq. 41: precedência
        for k in Pred[j]:
    ...
    # Eq. 37: cobertura por skill nas tasks LIVRES (as fixas já satisfazem via o
    # incumbente factível). Cada skill h coberta por ≥1 dev alocado (HdT_ijh).
    HdT = inst["HdT"]
    for jf in free:
        for h in range(inst["H_size"]):
            m.addConstr(gp.quicksum(HdT[i][jf][h] * w[i, jf, s]
                                    for (i, s) in poss_is[jf]) >= 1)
