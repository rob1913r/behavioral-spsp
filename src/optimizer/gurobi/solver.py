...
grava data/results/gurobi/<Cenario>/allocations.csv.

ATENÇÃO: este é o solver validado — a lógica do modelo NÃO deve ser
alterada (os resultados em data/results/gurobi/ levam horas para regerar).
"""
import os
import json
import glob
...
def solve_gurobi(file_path):
    ...
    if model.SolCount > 0:
        ...
        os.makedirs(f"data/results/gurobi/{nome_cenario}", exist_ok=True)
        allocs = []
        for i in M:
            for j in N:
                for s in S:
        ...
        pd.DataFrame(allocs).to_csv(f"data/results/gurobi/{nome_cenario}/allocations.csv", index=False)
