...
def load_instance(file_path: str) -> dict:
    ...
    Eq. 1: f_tech  = max(0.1, 1 - 0.5 * sum(max(0, Hd-Ht)^2) / (|H|*25))
    Eq. 2: psi     = sum(|Pd - Pt|) / (|P| * 4)     [semântica optimizer.py]
    ...
    for i in range(M):
        for j in range(N):
            et = (cfg.F_TECH_WEIGHT
                  * sum(max(0, Hd[i][h] - Ht[j][h]) ** 2 for h in range(H_size))
                  / (H_size * cfg.F_TECH_NORM))
            F_tech[i][j] = max(cfg.F_TECH_MIN, 1.0 - et)
            psi = (sum(abs(Pd[i][p] - Pt[j][p]) for p in range(P_size))
                   / (P_size * cfg.PSI_NORM_PER_QUAD))
    ...
    # --- HdT (Eq. 37 do artigo): dev i atende TODOS os requisitos de j ---
    HdT = [[1 if all(Hd[i][h] >= Ht[j][h] for h in range(H_size)) else 0
    ...
    # --- Prazo por task (Eq. 36). As instâncias atuais não trazem prazos
    #     individuais, então S_max_j = S_max (sem prazo apertado). ---
    inst["S_max_j"] = [inst["S_max"]] * N
...
def evaluate(inst, sa, fm) -> Evaluation:
    ...
    Semântica idêntica ao optimizer.py:
      ...
      Eq. 29: T_din  = T_sta * frac * F_total + Setup
              (T_aux = T_sta*frac*F_total tem lb=0 no Gurobi; aqui o valor
               negativo é clampado a 0 e a Eq. 29 acusa a violação)
    ...
    for j in range(N):
        ...
        for i in teams[j]:
            ...
            T_aux[i][j] = t_aux
            t_din = (t_aux if t_aux > 0.0 else 0.0) + cfg.SETUP_HOURS
            T_din[i][j] = t_din
            if t_din > dur_j:
                dur_j = t_din
        Dur[j] = dur_j
...
def r29_tdin_definition(inst, sa, fm, av):
    """Eq. 29 (h): T_din - [T_sta·(1/12)·sum_f f·gamma + w·Setup] = 0.
    Recomputa o tempo dinâmico. ATENÇÃO: no Gurobi T_aux tem lb=0, ou seja,
    T_sta·frac·F_total < 0 é INFACTÍVEL no MILP — aqui isso é acusado como
    violação (o VNS clampa em 0, mas o Gurobi rejeitaria a solução)."""
    out = []
    for j in range(inst["N"]):
        for i in _team(fm, j, inst["M"]):
            t_aux = inst["T_sta"][i][j] * fm[j][i] * av.F_total[i][j]
            if t_aux < -TOL:
                out.append((f"T{j}/Dev_{i}: T_aux={t_aux:.4f} < 0 "
                            f"(F_total={av.F_total[i][j]:.4f} negativo — "
                            f"infeasible no Gurobi)", -t_aux))
                continue
...
def r31_z_equals_sum_x(inst, sa, fm, av):
    ...
    task está em uma única sprint). Obs.: o optimizer.py usa a forma
    relaxada z <= sum_i w; com Eq.30+32 as duas são equivalentes."""
    out = []
    for j in range(inst["N"]):
...
def r37_skill_coverage(inst, sa, fm, av):
    """Eq. 37 (g): 1 - sum_{i,s} HdT_ij·w_ijs <= 0  ∀j,h com Ht_jh > 0.
    Pelo menos um dev plenamente qualificado por task com requisito.
    *** MODO WARN: esta restrição NÃO existe no optimizer.py (baseline
    Gurobi) — ver planning/proximos_passos.md. ***"""
    out = []
    for j in range(inst["N"]):
        has_requirement = any(inst["Ht"][j][h] > 0 for h in range(inst["H_size"]))
        if not has_requirement:
            continue
        cobertos = sum(inst["HdT"][i][j] for i in _team(fm, j, inst["M"]))
        g = 1 - cobertos
        if g > TOL:
            out.append((f"T{j}: nenhum dev alocado atende todos os "
                        f"requisitos tecnicos", float(g)))
    return out
